...
MIRROR_URL = f"https://api.github.com/repos/{MIRROR_REPO}/contents/{MIRROR_PATH}?ref={MIRROR_COMMIT}"
...
def download(source="mirror", country="India", year=2025, output: Path | None = None) -> Path:
    ...
    url = MIRROR_URL if source == "mirror" else official
    ...
    with httpx.Client(verify=ssl.create_default_context(), timeout=httpx.Timeout(120, connect=15), follow_redirects=True) as client:
        with client.stream("GET", url, headers={"Accept": "application/vnd.github.raw+json" if source == "mirror" else "text/csv", "User-Agent": "ThermoScan-research-prototype/0.1"}) as response:
